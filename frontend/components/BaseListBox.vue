<template>
  <div class="w-100 border border-gray-300 rounded">
    <Listbox v-model="selectedPerson">
      <div class="relative mt-1">
        <ListboxButton
          ...
          <span class="block truncate text-gray-900">{{ selectedPerson.label }}</span>
        ...
        <transition leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100"
          ...
          <ListboxOptions
            class="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-gray-50 py-1  shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
            <ListboxOption v-slot="{ active, selected }" v-for="person in people" :key="person.label" :value="person"
              as="template">
              <li :class="[
